...
for split in SPLITS:
    ...
    for index_ed in range(0,NUMBER_MCES_FOLDERS):
        edit_distance_data= LoadMCES.load_raw_data(directory_path=data_folder+INPUT_FOLDER_ED_format + str(index_ed),
        ...
        df_edit_distance_data_all['new_key']= df_edit_distance_data_all.apply(lambda x:(str(x['index_0'])+'_' + str(x['index_1'])), axis=1).astype(str)
        ...
        mces_data= LoadMCES.load_raw_data(directory_path=data_folder+INPUT_FOLDER_ED_format + str(index_ed),
        ...
        print('Mergin...')
        df_joint = pd.merge(df_mces_data, df_edit_distance_data_all, on='new_key', suffixes=('', '_drop'))
        ...
        path_file = OUTPUT_FOLDER + 'indexes_tani_incremental' + split + '_' + str(index_ed)+ '.npy'
