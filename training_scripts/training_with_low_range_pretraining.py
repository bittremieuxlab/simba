...
dataloader_val_low_range = DataLoader(
    dataset_val_low_range,
    batch_size=config.BATCH_SIZE,
    worker_init_fn=worker_init_fn,
    num_workers=0,
)
...
## LOW RANGE PRETRAINING
trainer_low_range = pl.Trainer(
    max_epochs=10,
    callbacks=[checkpoint_callback, losscallback_pretrain],
    enable_progress_bar=enable_progress_bar,
    # val_check_interval= config.validate_after_ratio,
)
# trainer = pl.Trainer(max_steps=100,  callbacks=[checkpoint_callback, losscallback], enable_progress_bar=enable_progress_bar)
trainer_low_range.fit(
    model=model,
    train_dataloaders=(dataloader_train_low_range),
    val_dataloaders=dataloader_val_low_range,
)
