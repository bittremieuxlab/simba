...
from src.transformers_classification.load_data_class import LoadDataClass
import lightning.pytorch as pl
from src.transformers_classification.embedder_class import EmbedderClass
...
## Load the computed tanimoto files
molecule_pairs_train.indexes_tani = np.load(output_np_indexes_train)
...
samples_per_range, bins = SanityChecks.check_distribution_similarities(
    molecule_pairs_train, bins=config.bins_uniformise_TRAINING
)
...
dataset_train = LoadDataClass.from_molecule_pairs_to_dataset(m_train, training=True)
# dataset_test = LoadData.from_molecule_pairs_to_dataset(m_test)
dataset_val = LoadDataClass.from_molecule_pairs_to_dataset(m_val)
...
if config.load_pretrained:
    model = EmbedderClass.load_from_checkpoint(
