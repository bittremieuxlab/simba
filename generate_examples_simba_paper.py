...
#indexes_tani_multitasking_test = indexes_tani_multitasking_test[0:10000]
...
'''
print('Running MS2Deepscore')
similarity_model_ms2= MS2DeepScore(model_ms2d)
pred_ms2=[]
for s0, s1 in zip(spectra0_mms, spectra1_mms):
    results_scores = calculate_scores(
                            [s0], [s1], similarity_model_ms2
                        )
    similarity_retrieved= results_scores.to_array().transpose()[0][0]

    #retrieved_sim=np.array(similarity_retrieved[0] if similarity_retrieved[1]>=6 else 0)
    retrieved_sim=np.array(similarity_retrieved)
    pred_ms2.append(retrieved_sim)
pred_ms2= np.array(pred_ms2)
'''


print('Running modified cosine')
similarity_model =ModifiedCosine(tolerance=0.1)
pred_mod_cos=[]
for s0, s1 in zip(spectra0_mms, spectra1_mms):
    results_scores = calculate_scores(
                            [s0], [s1], similarity_model
                        )
    similarity_retrieved= results_scores.to_array().transpose()[0][0]

    #retrieved_sim=np.array(similarity_retrieved[0] if similarity_retrieved[1]>=6 else 0)
    retrieved_sim=np.array(similarity_retrieved[0])
    pred_mod_cos.append(retrieved_sim)
pred_mod_cos= np.array(pred_mod_cos)



print('Running tanimotos ...')
tanimotos = [Tanimoto.compute_tanimoto_from_smiles(s0.params['smiles'],s1.params['smiles']) for s0,s1 in zip(spectra0, spectra1)]


## Get interesting examples
def filter_good_examples(similarities1, predictions1, similarities2, predictions2,pred_mod_cos,,tanimotos):
        ...
        #low_ms2 = (pred_ms2< 0.2)


        condition_1= (equal_edit_distance  & equal_mces & low_mces)  & low_mod_cos 
        condition_2= (equal_edit_distance  & equal_mces & low_mces)  & low_tanimoto 
        #condition_3= (equal_edit_distance  & equal_mces & low_mces)  & low_ms2 

        return np.argwhere(condition_1|condition_2 )
print(f'Tanimotos {tanimotos}')
tanimotos = [t if t is not None else np.nan for t in tanimotos ]
tanimotos =np.array(tanimotos)
good_indexes = filter_good_examples(similarities_test1, flat_pred_test1, similarities_test2, flat_pred_test2,pred_mod_cos,pred_ms2, tanimotos)

print(f'Total examples found:{good_indexes}')
...
PerformanceMetrics.plot_molecules(uniformed_molecule_pairs_test_mces, prediction_results,  good_indexes, config, prefix='good')
