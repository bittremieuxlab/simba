...
#TRAINING_SUBSET= 2000000  #or None if not debugging
...
USING_UC_EXTRA_DATA=True
if USING_UC_EXTRA_DATA:
    try:
...
dataset_train = LoadDataMultitasking.from_molecule_pairs_to_dataset(
    molecule_pairs_train, max_num_peaks=int(config.TRANSFORMER_CONTEXT), training=True, use_fingerprints=config.USE_FINGERPRINT,
)
# dataset_test = LoadData.from_molecule_pairs_to_dataset(m_test)
dataset_val = LoadDataMultitasking.from_molecule_pairs_to_dataset(
    molecule_pairs_val, max_num_peaks=int(config.TRANSFORMER_CONTEXT), use_fingerprints=config.USE_FINGERPRINT,
)
