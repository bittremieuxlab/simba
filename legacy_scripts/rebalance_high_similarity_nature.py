...
for k in ["molecule_pairs_train", "molecule_pairs_val"]:
    ...
    number_additional_pairs = int(
        PROPORTION_ADDITIONAL_PAIRS * target_dataset.indexes_tani.shape[0]
    )
    ## Check proportion of high similarity pairs > 0.90
    high_train_data = target_dataset.indexes_tani[
        target_dataset.indexes_tani[:, 2] == 1
    ]
    low_train_data = target_dataset.indexes_tani[
        target_dataset.indexes_tani[:, 2] < 0.5
    ]

    ## Now get the same spectrums but in the low range
    high_range_indexes = np.concatenate(
        (high_train_data[:, 0], high_train_data[:, 1]), axis=0
    )
    ...
    new_indexes_tani = np.concatenate(
        (target_dataset.indexes_tani, matched_data), axis=0
    )
