...
def get_formula_subformulas(spec, use_best_subformula=True, USE_PERFECT_FORMULA=False, USE_3_FORMULAS=True):
    if USE_PERFECT_FORMULA:
        formula_results = formula_from_smiles(spec.params['smiles'])
        top=formula_results
    else:
        formula_results,top, results_formula = predict_formula_msms(mz_list = spec.mz,
    ...
    if use_best_subformula:
        ...
        for k in keys:
            ...
            if results_formula[k] is not None:
                ...
                subformula_results[k] = [{'best_subformula':None} for s in subformula_results['formula_rank_1']]
        subformula_results = [ [subformula_results[k][n]['best_subformula'] for k in keys] for n in range(0, len(subformula_results['formula_rank_1']))]
...
from msbuddy.base import Spectrum, MetaFeature      # in-memory objects
...
def predict_formula_msms(
    ...
    ms2_spec = Spectrum(np.asarray(mz_list,  dtype=float),
...
def predict_subformulas_msms(
    ...
    if sub_results is not None:
        ...
        for res in sub_results:
            ...
            sub_list = [
                sf if isinstance(sf, str) else   # already a string?
                (sf.formula if hasattr(sf, "formula") else form_arr_to_str(sf))
...
def core_struct_features_from_smiles(smiles: str):
    ...
    if mol is None:
        ...
        mol = Chem.AddHs(mol, addCoords=False)  # keeps formula/charge consistent
