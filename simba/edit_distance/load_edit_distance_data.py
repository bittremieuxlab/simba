...
class LoadEditDistanceData:
    ...
    def load_file(
        ...
        try:
            df_rs = pd.read_csv(path_file, names=names, on_bad_lines="warn")
        except:
            print(
                "Error using default engine, trying python (slower but more effective)"
            )
            # load all the lines
            df_rs = pd.read_csv(
                path_file,
            ...
            df_rs["raw_data"] = df_rs["raw_data"].apply(
                lambda x: x.split('"')[0] + "False"
            )
    ...
    def get_matched_rows(df_rs, specs):
        ...
        our_canon_smiles_dict = {
            s.params["smiles"]: Chem.CanonSmiles(s.params["smiles"]) for s in specs
        }
        our_canon_smiles_list = [
            our_canon_smiles_dict[k] for k in our_canon_smiles_dict
        ]

        # going from canon to non canon in the spec domain
        from_canon_to_no_canon = {
            our_canon_smiles_dict[k]: k for k in our_canon_smiles_dict
        }
        ...
        canon_unique_ming_smiles_0 = LoadEditDistanceData.get_canon(
            unique_ming_smiles_0
        )
        canon_unique_ming_smiles_1 = LoadEditDistanceData.get_canon(
            unique_ming_smiles_1
        )

        # get the uniquelist of smiles
        list_unique_canon_smiles_0 = [
            canon_unique_ming_smiles_0[k] for k in canon_unique_ming_smiles_0
        ]
        list_unique_canon_smiles_1 = [
            canon_unique_ming_smiles_1[k] for k in canon_unique_ming_smiles_1
        ]

        ## Find the unique matches in the unique list
        matched_smiles_0 = [
            s for s in our_canon_smiles_list if s in list_unique_canon_smiles_0
        ]
        ## Find the matches in ming_smiles_1
        matched_smiles_1 = [
            s for s in our_canon_smiles_list if s in list_unique_canon_smiles_1
        ]

        # dictionary indexed by canon smiles to indexes in the spec domain
        unique_indexes_tani_0 = {
            s: our_smiles.index(from_canon_to_no_canon[s]) for s in matched_smiles_0
        }
        unique_indexes_tani_1 = {
            s: our_smiles.index(from_canon_to_no_canon[s]) for s in matched_smiles_1
        }

        # modify df_rs
        df_rs["canon_smiles_0"] = df_rs["smiles_0"].apply(
            lambda x: canon_unique_ming_smiles_0[x]
        )
        df_rs["canon_smiles_1"] = df_rs["smiles_1"].apply(
            lambda x: canon_unique_ming_smiles_1[x]
        )
        matched_dict_0 = {
            s: (s in matched_smiles_0) for s in canon_unique_ming_smiles_0
        }
        matched_dict_1 = {
            s: (s in matched_smiles_1) for s in canon_unique_ming_smiles_1
        }
        ...
        df_rs_filtered.loc[:, "indexes_tani_0"] = df_rs_filtered[
            "canon_smiles_0"
        ].apply(lambda x: unique_indexes_tani_0[x])
        df_rs_filtered.loc[:, "indexes_tani_1"] = df_rs_filtered[
            "canon_smiles_1"
        ].apply(lambda x: unique_indexes_tani_1[x])
    ...
    def decompress_data(file_name):
        ...
        print("Decompressing ...")
        LoadEditDistanceData.decompress_gz(
            input_gz_file=file_name, output_file=csv_file
        )
    ...
    def forward(
        ...
        for index, l in tqdm(enumerate(list_files)):
            ...
            for k in dict_save:
                ...
                file_name = (
                    output_folder
                    + f"indexes_tani_incremental_{dict_save[k]}_{str(index)}.npy"
                )
