...
class LoadEdistanceDataInchi:
    ...
    def generate_smiles_csv(
        ...
        df["smiles_0"] = df["inchi1"].apply(
            lambda x: LoadEdistanceDataInchi.get_smiles_from_inchi_rdkit(x)
        )
        df["smiles_1"] = df["inchi2"].apply(
            lambda x: LoadEdistanceDataInchi.get_smiles_from_inchi_rdkit(x)
        )
        df["inchi_0"] = ""
        df["inchi_1"] = ""
    ...
    def generate_smiles_and_save(df, index, output_folder, csv_new_names):
        ...
        df_output = LoadEdistanceDataInchi.generate_smiles_csv(
            df,
            columns=csv_new_names,
        )
    ...
    def forward(
        ...
        for chunk_index, df_chunk in tqdm(enumerate(generator)):
            ...
            LoadEdistanceDataInchi.generate_smiles_and_save(
                df_chunk, chunk_index, output_folder, csv_new_names
            )
