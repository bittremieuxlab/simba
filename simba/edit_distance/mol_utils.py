...
def get_modification_edges(mol1, mol2, only_outward_edges=False):
    ...
    if not mol1.HasSubstructMatch(mol2):
        if mol2.HasSubstructMatch(mol1):
            ...
            raise ValueError(
                "One molecule should be a substructure of the other molecule"
            )

    matches = _find_minimal_modification_edges_match(copy_mol1, copy_mol2)
    modificationEdgesOutward, modificationEdgesInside, _ = _get_edge_modifications(
        copy_mol1, copy_mol2, matches
    )
...
def get_transition(input1, input2):
    ...
    modified_removed_edges_bridge, modified_removed_edges_inside, removed_edges = (
        _get_edge_modifications(mol1, mcs_mol, mol1_indices)
    )

    # calculating the added atoms from mol2
    added_atoms = set(range(mol2.GetNumAtoms())) - set(mol2_indices)
    modified_added_edges_bridge, modified_added_edges_inside, added_edges = (
        _get_edge_modifications(mol2, mcs_mol, mol2_indices)
    )
    ...
    for bond in modified_added_edges_inside:
        editable_mol.AddBond(
            map_old_mol2_to_added[bond[0]],
            map_old_mol2_to_added[bond[1]],
            order=copy_mol2.GetBondBetweenAtoms(bond[0], bond[1]).GetBondType(),
        )
        updated_modified_added_edges_inside.append(
            (map_old_mol2_to_added[bond[0]], map_old_mol2_to_added[bond[1]])
        )
    ...
    for bond in mol1.GetBonds():
        ...
        if (
            pair in modified_removed_edges_inside
            or pair in modified_removed_edges_bridge
            or pair in removed_edges
        ):
            continue
        pair2 = (bond.GetEndAtomIdx(), bond.GetBeginAtomIdx())
        if (
            pair2 in modified_removed_edges_inside
            or pair2 in modified_removed_edges_bridge
            or pair2 in removed_edges
        ):
            continue
...
def get_modification_graph(main_struct, sub_struct):
    ...
    atoms_of_substructure = _find_minimal_modification_edges_match(
        main_struct, sub_struct
    )
    (
        modificationEdgesOutward,
        modificationEdgesInside,
        noneModificationDifferentEdges,
    ) = _get_edge_modifications(main_struct, sub_struct, atoms_of_substructure)
    ...
    for modification in range(color):
        ...
        for atom in range(main_struct.GetNumAtoms() - 1, -1, -1):
            if atom not in visited or visited[atom] != modification:
                ...
                edit_mol.GetAtomWithIdx(atom).SetProp(
                    "atomNote", f"substructure_{atom}"
                )
...
def attach_mols(
    ...
    new_mol.AddBond(
        attach_location_main,
        attach_location_attachment + main_mol.GetNumAtoms(),
        bond_type,
    )
...
def generate_possible_stuctures(main_struct, sub_struct):
    ...
    bondType = (
        all_modifications[0][0]
        .GetBondBetweenAtoms(wild_atom, neighbor.GetIdx())
        .GetBondType()
    )
    ...
    for atom in sub_struct.GetAtoms():
        temp_struct = _attach_struct_try(
            sub_struct, frag, atom.GetIdx(), index_in_frag, bondType
        )
    ...
    try:
        ...
        for bond in [Chem.BondType.SINGLE, Chem.BondType.DOUBLE, Chem.BondType.TRIPLE]:
            if bond != bond_type:
                try:
                    mol = attach_mols(
                        main_struct, frag, main_location, frag_location, bond
                    )
                    Chem.SanitizeMol(mol)
                    return mol
                except Exception:
...
def _find_minimal_modification_edges_match(mol, substructure):
    ...
    if len(matches) == 1:
        ...
        min_match = None
        for match in matches:
            modificationEdgesOutward, modificationEdgesInside, _ = (
                _get_edge_modifications(mol, substructure, match)
            )
            num_modifications = len(modificationEdgesOutward) + len(
                modificationEdgesInside
            )
        ...
        if bond.GetBeginAtomIdx() in intersect or bond.GetEndAtomIdx() in intersect:
            if (
                ...
                if substructure.GetBondBetweenAtoms(a1, a2) is None:
                    modificationEdgesInside.append(
                        (bond.GetBeginAtomIdx(), bond.GetEndAtomIdx())
                    )
            else:
                modificationEdgesOutward.append(
                    (bond.GetBeginAtomIdx(), bond.GetEndAtomIdx())
                )
        else:
            noneModificationDifferentEdges.append(
                (bond.GetBeginAtomIdx(), bond.GetEndAtomIdx())
            )
        ...
        if type(matches[0]) is tuple:
            ...
            for match in matches:
                ...
                for atom in modificationSites:
                    idx = subMatches.index(
                        atom
                    )  # based on rdkit library, the value in matches array are sorted by index
                    temp_res.append(idx)
                res.append(temp_res)
        else:
            for atom in modificationSites:
