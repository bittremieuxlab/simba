...
class EmbedderMultitask(Embedder):
    ...
    def __init__(
        ...
        if self.use_gumbel:
            ...
            print(f"Usign TAU GUMBEL reg weight: {self.gumbel_reg_weight}")
        ...
        self.linear2_cossim = nn.Linear(
            d_model, d_model
        )  # extra linear transformation in case cosine similarity i sused
    ...
    def compute_adjacent_diffs(self, gumbel_probs_1, batch_size):
        ...
        diff_penalty = (
            torch.sum(
                squared_adjacent_diffs
            )  # Sum of differences between adjacent columns
    ...
    def step(
        ...
        target1 = target1.view(
            -1
        )  # Ensure targets are in the right shape and type for classification
    ...
    def step_mse(self, batch, batch_idx, threshold=0.5):
        ...
        target = target.view(
            -1
        ).float()  # Ensure targets are in the right shape and type for regression
        # print('target')
        # print(target[0])
