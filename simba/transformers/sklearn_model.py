...
class SklearnModel(BaseEstimator, ClassifierMixin):
    ...
    def x_to_item(self, x, size_per_key):
        ...
        for n in range(x.shape[0]):
            ...
            for k in size_per_key:
                ...
                if k != "similarity":
                    valid_columns = [
                        column for column in x.keys() if column.startswith(k)
                    ]
                else:
                    valid_columns = [
                        column
                        ...
                        if (
                            column.startswith("similarity")
                            and (not (column.startswith("similarity2")))
                        )
                    ]
    ...
    def get_X_from_all_molecule_pairs(self, all_molecule_pairs):
        ...
        if self.multitasking:
            dataset_test = LoadDataMultitasking.from_molecule_pairs_to_dataset(
                all_molecule_pairs, max_num_peaks=self.max_peaks
            )
        else:
            dataset_test = LoadDataUnique.from_molecule_pairs_to_dataset(
                all_molecule_pairs
            )
    ...
    def compute_shap_values(self, explainer, one_molecule_pair):
        ...
        if self.multitasking:
            ...
            dataset_test = LoadDataMultitasking.from_molecule_pairs_to_dataset(
                one_molecule_pair, max_num_peaks=self.max_peaks
            )
        else:
            dataset_test = LoadDataUnique.from_molecule_pairs_to_dataset(
                all_molecule_pairs
