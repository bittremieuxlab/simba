...
class LoadDataBase:
    ...
    def load_spectrum_data(
        ...
        spectrums = [copy.deepcopy(s) for s in spectrums_input]
        ...
        mz = np.zeros((len(spectrums), max_num_peaks), dtype=np.float32)
        intensity = np.zeros((len(spectrums), max_num_peaks), dtype=np.float32)
        precursor_mass = np.zeros((len(spectrums), 1), dtype=np.float32)
        precursor_charge = np.zeros((len(spectrums), 1), dtype=np.int32)
