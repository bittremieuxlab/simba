...
config.PREPROCESSING_DIR_TRAIN = '/Users/sebas/projects/data/preprocessing_ed_mces_20250123/'
...
def create_new_spectra(spectrums):
    ...
    for s in spectrums:
        ...
        new_spectrum = Spectrum(mz=s.mz, intensities=s.intensities, metadata=new_metadata)
...
for s0, s1 in tqdm(zip(spectra0_mms, spectra1_mms)):
    ...
    results_scores_mod = calculate_scores(
                            [s0], [s1], similarity_model_mod
                        )
...
def filter_good_examples(similarities1, predictions1, similarities2, predictions2,pred_mod_cos,pred_ms2, tanimotos):
        ...
        low_ms2 = (np.abs(pred_ms2 -tanimotos)>0.5) &  (pred_ms2<0.5)## the prediction is low and the differrence between tanimoto and ms2 is high


        condition_1= (equal_edit_distance  & equal_mces & low_mces) & low_tanimoto  & low_mod_cos  
        condition_2= (equal_edit_distance  & equal_mces & low_mces)  &low_tanimoto  & high_mod_cos  
        condition_3 = (equal_edit_distance  & equal_mces & low_mces) & high_tanimoto & low_mod_cos
        condition_4 = (equal_edit_distance  & equal_mces & low_mces) & high_tanimoto & high_mod_cos
        ...
        good_indexes_dict['bad_ms2_high_tani_low_mod']=np.argwhere(condition_5 & condition_3)
...
good_indexes_dict = filter_good_examples(similarities_test1, flat_pred_test1, similarities_test2, flat_pred_test2,pred_mod_cos, pred_ms2, tanimotos)
