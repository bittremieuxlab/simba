...
# %%
all_spectrums_reference_su = (
    dataset["molecule_pairs_train"].spectrums_original
...
# %%
all_spectrums_reference_matchms = (
    all_spectrums_gnps_matchms + all_spectrums_nist_matchms
)
...
all_spectrums_reference = [
    all_spectrums_reference_matchms[index] for index in indexes_matched
]

# %%
for i, (s_ref, s_su) in enumerate(
    zip(all_spectrums_reference, all_spectrums_reference_su)
):
...
all_spectrums_janssen = [
    s for s in all_spectrums_janssen_matchms if s.spectrum_hash() in su_hashes
]
...
# %%
canon_smiles_reference = [
    Chem.CanonSmiles(s.metadata["smiles"]) for s in all_spectrums_reference
]
canon_smiles_janssen = [
...
# %%
all_spectrums_janssen = [
    all_spectrums_janssen[index] for index in janssen_indexes_not_in_ref
]
...
preprocessed_all_spectrums_janssen = [
    ...
    if ((s is not None) and (s.metadata["precursor_mz"] > 0))
]
preprocessed_all_spectrums_janssen = [
    s
    for s in preprocessed_all_spectrums_janssen
...
if PREPROCESS_SPECTRUMS:
    for i, s in tqdm(enumerate(preprocessed_all_spectrums_reference)):
        preprocessed_all_spectrums_reference[i] = (
            Spec2VecComparison.spectrum_processing(
                preprocessed_all_spectrums_reference[i]
            )
        )
preprocessed_all_spectrums_reference = [
    s
...
if similarity_name != "ms2deepscore":
    ...
    results_tuple = [
        results_scores.scores_by_query(s, name="Spec2Vec", sort=True)[0]
        for s in preprocessed_all_spectrums_janssen
    ]
    ...
    results_tuple = [
        results_scores.scores_by_query(s, name="MS2DeepScore", sort=True)[0]
        for s in preprocessed_all_spectrums_janssen
    ]
...
# %%
tanimoto_retrieved = [
    Tanimoto.compute_tanimoto_from_smiles(s0, s1)
    for s0, s1 in zip(smiles_janssen, smiles_retrieved)
]
...
# %%
## mcs
sim_mcs, mol_mcs = calculate_mcs_similarity(
    smiles1=all_spectrums_janssen[target_spectra].metadata["smiles"],
...
mol_janssen = Chem.MolFromSmiles(
    all_spectrums_janssen[target_spectra].metadata["smiles"]
)
mol_janssen
...
# %%
results = {
    "preprocessed_all_spectrums_janssen": preprocessed_all_spectrums_janssen,
...
# %%
with open(
    "./notebooks/discovery_search/results/"
    + similarity_name
    + "_results_analog_discovery_unknwon_compounds.pkl",
    "wb",
) as f:
    dill.dump(results, f)
...
target_spectrum_janssen = [
    s
    for s in preprocessed_all_spectrums_janssen
    if s.metadata["id"] == "CUIHSIWYWATEQL"
][0]
...
# %%
target_results = calculate_scores(
    [target_spectrum_ref], [target_spectrum_janssen], similarity_model
)
