        ...
        for target_query_index in target_indexes_list[0:number_samples]:
            ...
            for model_used_key in model_results:
                ...
                precursor_mz_retrieved = np.array([
                    Chem.Descriptors.ExactMolWt(Chem.MolFromSmiles(s)) for s in smiles_retrieved
                ])
                ...
                axes[model_idx, 0].bar(x_ranking, mces_distance_list, label="MCES Distance")
                axes[model_idx, 0].plot(
                    x_ranking, best_smiles_vector, marker="*", color="r", label="Best SMILES"
                )
                axes[model_idx, 0].set_title(f'Median distance: {np.median(mces_distance_list):.2f}')
                ...
                axes[model_idx, 1].bar(x_ranking, sim_k_retrieved, label="Model Score", color="g")
                ...
                axes[model_idx, 2].set_title(f' Median diff mz: {np.median(diff_mz):.2f}')
                ...
                axes[model_idx, 0].set_ylabel(f'{model_used_key}\n\n' + axes[model_idx, 0].get_ylabel(), fontsize=12)
                model_idx += 1

            # Adjust layout and save the figure
            plt.tight_layout()
            plt.savefig(os.path.join(folder_path, f'{target_query_index}_combined_plot.png'))
            plt.show()
    ...
    def plot_performance(model_results, 
        ...
        for model_used_key in model_results:
            ...
            for target_query_index in target_indexes_list:
                ...
                plt.show()
    ''' 
    def obtain_precision_k_candidates(golden_truth, 
                                    predicted, 
                                    smiles_reference,
                                    smiles_janssen,
                                    all_spectrums_reference, 
        ...
        for min_tanimoto in min_tanimotos:
            ...
            for k in k_list:
                ...
                for i in range(0, len(all_spectrums_janssen)):
                    ...
                    best_smile = all_spectrums_reference[best_indexes[i]].params['smiles']
                    best_smile = Chem.CanonSmiles(best_smile)
                    ...
                    if best_tanimoto>min_tanimoto:
                        ...
                        retrieved_smiles = [smiles_reference[int(b)] for b in best_retrieved_indexes]
                        
                        retrieved_smiles=[Chem.CanonSmiles(s) for s in retrieved_smiles]
