...
class AnalogDiscovery:
    ...
    def get_analog_spectrums_matchms(results_scores, spectrums_reference, spectrums_query, ):
        '''
        based on finding k=10 best ranked spectrums and then finding the best tanimoto
        '''
        spectrums_k_retrieved, max_k_sim= AnalogDiscovery.get_k_ranking_matchms(results_scores, spectrums_query)
    ...
    def get_analog_spectrums_su(similarities, spectrums_reference, spectrums_query, k=10):
        ...
        spectrums_retrieved = [s_group[ind] for ind,s_group in zip(best_index_retrieved,spectrums_k_retrieved)]
        tanimoto_retrieved = [max(t) for t in tanimoto_k_retrieved]
        max_sim = [s_group[ind] for ind,s_group in zip(best_index_retrieved,sorted_k_similarities)]
    ...
    def get_k_ranking_matchms(results_scores, spectrums_query, k=10):
        ...
        results_tuple = [results_scores.scores_by_query(s, name='ModifiedCosine_score', sort=True)[0:k] for s in spectrums_query]
    ...
    def get_best_tanimoto_from_k_ranking(spectrums_reference, spectrums_query, spectrums_k_retrieved, max_k_sim):
        '''
        get the best tanimoto score from a set of spectrums
        '''
        # compute all tanimotos
        smiles_k_retrieved = [[s.metadata['smiles'] for s in s_group] for s_group in spectrums_k_retrieved]
        smiles_k_janssen =   [ s.metadata['smiles'] for s in spectrums_query]
        tanimoto_k_retrieved = [[Tanimoto.compute_tanimoto_from_smiles(s0,s1) for s1 in s1_group] for s0, s1_group in zip(smiles_k_janssen, smiles_k_retrieved)]
    ...
    def get_rank_of_best_candidate(similarities, smiles_janssen, smiles_janssen_loaded, smiles_reference, smiles_best_candidate_loaded):
        ...
        for s_janssen, arg_sim_row in zip(smiles_janssen, argsort_similarities):
            ...
            if s_janssen in smiles_janssen_loaded:
                ...
                best_candidate_smile = smiles_best_candidate_loaded[smiles_mapped_loaded_index]
