...
class LoadDataUnique:
    ...
    def from_molecule_pairs_to_dataset(
        ...
        molecule_pairs = MoleculePairsOpt(
            spectrums_original=[copy.copy(s) for s in molecule_pairs_input.spectrums_original], 
        ...
        molecule_pairs.spectrums_original = pp.preprocess_all_spectrums(molecule_pairs.spectrums_original)

        print("Finished preprocessing ")

        ## Get the mz, intensity values and precursor data
        mz = np.zeros((len(molecule_pairs.spectrums_original), max_num_peaks), dtype=np.float32)
        intensity = np.zeros((len(molecule_pairs.spectrums_original), max_num_peaks), dtype=np.float32)
        precursor_mass = np.zeros((len(molecule_pairs.spectrums_original), 1), dtype=np.float32)
        precursor_charge = np.zeros((len(molecule_pairs.spectrums_original), 1), dtype=np.int32)
        ...
        for i, l in enumerate(molecule_pairs.spectrums_original):
            ...
            length = (
                len(l.mz)
                if len(l.mz) <= max_num_peaks
                else max_num_peaks
            )

            # assign the values to the array
            mz[i, 0:length] = np.array(l.mz[0:length])
            intensity[i, 0:length] = np.array(
                l.intensity[0:length]
            )
        ...
        return CustomDatasetUnique(dictionary_data, 
                                   training=training,
                                   mz=mz,
                                   intensity=intensity,
